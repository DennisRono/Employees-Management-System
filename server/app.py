...
class EmployeeResource(Resource):
    ...
    def delete(self, employee_id):        
        ...
        db.session.commit()
        return make_response(jsonify({"message": f"Employee {employee.id} deleted"}), 204)
...
class DepartmentResource(Resource):
    ...
    def delete(self, department_id):        
        ...
        db.session.commit()
        return make_response(jsonify({"message": f"Department {department.id} deleted"}), 204)
...
class AdministratorResource(Resource):
    def get(self, admin_id=None):
     ...
     if admin_id:
         administrator= Administrator.query.filter(Administrator.id==admin_id).first()
         if administrator:
             return make_response(jsonify(administrator.to_dict()), 200)
         else:
             return make_response(jsonify({"error":f"Admistrator {administrator.id} not found"}), 404)
     else:
         administrators=Administrator.query.all()
         return make_response(jsonify([admin.to_dict() for admin in administrators]), 200)
     
    def post(self):        
        data = request.get_json()
        new_admin = Administrator(admin_name=data["customer_name"], email=data["email"], phone_number=data["phone_number"])
    ...
    def delete(self, admin_id):        
        ...
        db.session.commit()
        return make_response(jsonify({"message": f"Administrator {admin.id} deleted"}), 204)
